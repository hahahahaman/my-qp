...
(defun write-system-form (name
  ...
  (let ((*print-case* :downcase))
    ...
    (format stream "~A\")~%" "                *************~%")
    (format stream "  (asdf:oos 'asdf:load-op ~S)~%" (uninterned-symbolize
                                                      (concatenate 'string
                                                                   name
                                                                   ".tests")))
    (format stream "  (asdf:oos 'asdf:test-op ~S))" (uninterned-symbolize
                                                     (concatenate 'string
                                                                  name
                                                                  ".tests")))))
...
(defun write-test-application-file (name file)
  (with-new-file (stream file)
    (file-comment-header stream)
    (format stream "(in-package ~S)~%~%" (uninterned-symbolize (concatenate 'string
                                                                            name
                                                                            ".tests")))
